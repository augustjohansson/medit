...
macosx:
	$(MAKE) $(MFLAGS) -f Makefile.arch \
        "prog   = medit" \
	"CC     = gcc-4.2" \
	"CFLAGS = -O3 -funroll-loops -ansi \
	          -fexpensive-optimizations \
	          -Wuninitialized -Wunused -Winline -Wshadow" \
	"LDFLAGS= $(HOME)/code/library.cur/objects/$(ARCHI)/*.o" \
	"LDFLAGS= " \
...
macosx-x64:
	...
	"CFLAGS = -O3 -arch x86_64 -Dppc -funroll-loops -ansi \
	          -fexpensive-optimizations \
	          -Wuninitialized -Wunused -Winline -Wshadow" \
	"LDFLAGS= -arch x86_64 $(HOME)/code/library.cur/objects/$(ARCHI)/*.o" \
	"INCDIR = -Isources -I/usr/include -I$(HOME)/code/library.cur/sources" \
	"GL_LIB = " \
...
linux:
	$(MAKE) $(MFLAGS) -f Makefile.arch \
        "prog   = medit" \
	"CC     = gcc" \
	"CFLAGS = -O3 -march=native -ansi -fexpensive-optimizations \
	          -Wuninitialized -Wunused -Winline -Wshadow" \
        "INCDIR = -I. -I./sources" \
        "GL_LIB   = /usr/lib/x86_64-linux-gnu/libGL.so" \
        "GLU_LIB  = /usr/lib/x86_64-linux-gnu/libGLU.so" \
        "GLUT_LIB = /usr/lib/x86_64-linux-gnu/libglut.so" \
        "EXEDIR = $(PWD)"
...
tar:
	tar czfL archives/medit.`date +"%Y.%m.%d"`.tgz sources makefile Makefile.arch
